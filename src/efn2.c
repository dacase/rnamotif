...
typedef	struct	efn2data_t	{
	...
	int	e2_iloop21[ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ];
	int	e2_iloop11[ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ];
	int	e2_iloop22[ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ][ N_BCODES ];
...
static	int	bmap[] = { BCODE_A, BCODE_C, BCODE_G, BCODE_T, BCODE_G, BCODE_T };
static	int	rmap[] = { BCODE_T, BCODE_G, BCODE_C, BCODE_A, BCODE_T, BCODE_G };

static	int	getmiscloop( char [] );
static	int	getibhloop( char [] );
static	int	getdangle( char [] );
static	int	getstack( char [], int [N_BCODES][N_BCODES][N_BCODES][N_BCODES], int );
static	int	getcoax( char [], int [N_BCODES][N_BCODES][N_BCODES][N_BCODES], int );
...
static	int	ef2_ibloop( int, int, int, int, int, int );
static	int	ef2_hploop( int, int, int );
static	int	ef2_dangle( int, int, int, int, int );
...
int	RM_getefn2data( void )
{
	...
	if( !gettstack( "tstack.dat", ef2dp->e2_tstack, ef2dp->e2_dangle, ef2dp->e2_auend ) )
		return( 0 );
	...
	int stack[N_BCODES][N_BCODES][N_BCODES][N_BCODES], int b5val ) 
{
	...
	for( v1 = 0; v1 < 4; v1++ ){
		if( !skipto( fp, "<--", sizeof( line ), line ) ){
			sprintf( emsg,
				"getcoax: premature end of stack file '%s'.",
				pname );
			RM_errormsg( 0, emsg );
			rval = 0;
			goto CLEAN_UP;
				...
				for( v4 = 0; v4 < N_BCODES; v4++ ){
					if((v1==BCODE_N)||(v2==BCODE_N)||(v3==BCODE_N)||(v4==BCODE_N)) {
						...
						if( *fields[ f ] == '.' ){
							...
							tstack[v1][v2][v3][v4]= NINT(100.0*atof( fields[f] ));
					}
				}
			}
		}
...
int	RM_efn2()
{
	...
	energy = 0;
	stack.sp = 0;
	push(&stack,0,rm_l_base,1,0);	/* this one is OK */
...
SUBROUTINE : ;
	...
	while( stz!=1 ){
		...
		while( rm_basepr[i]==j ){
			...
			if( sum==0 ){		/* hairpin loop	*/
				energy += ef2_hploop(i,j,0);
				goto SUBROUTINE;
			}else if( sum==1 ){	/* bulge/internal loop	*/
				energy += ef2_ibloop(i,j,ip,jp,0,0);
				...
				if( sum >= MAXHELIX ){
					...
					energy = EFN2_INFINITY;
					return( energy );
				}

				for( k=0; k<=sum; k++ )
					memset(coax[k],0,(sum+1)*sizeof(int));
				...
				for( k=1; k<sum; k++ ){
					...
					while( rm_basepr[ip] == UNDEF )
						ip++;
					/* add the terminal AU ef2_aupen if necessary	*/
					energy += ef2_aupen(ip,rm_basepr[ip]);
					helix[k][1] = ip;
					helix[k][0] = rm_basepr[ip];
					push( &stack,ip,rm_basepr[ip],1,0 );
					sum1 += ip - helix[k-1][0] - 1;
				}
				helix[sum][0] = helix[0][0];
				helix[sum][1] = helix[0][1];

				sum1 += helix[sum][1] - helix[sum-1][0] - 1;

				/* give the multibranch loop bonus:				*/
				energy += ef2dp->e2_efn2a;

				/* give the energy for each entering helix			*/
				energy += sum*ef2dp->e2_efn2c;

				if( sum1<=6 ){
					energy += sum1*(ef2dp->e2_efn2b);
				}else{
					energy +=
						6*(ef2dp->e2_efn2b)+(int)(11.*log((double)((sum1/6.)))+0.5);
				}
	...
	coax[ip][ip] = ef2dp->e2_tstkm[rm_bcseq[helix[ip][0]]][rm_bcseq[helix[ip][1]]][rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip][1]-1]];
							}else{
								if( helix[ip+1][1] - helix[ip][0] > 1 ){
	coax[ip][ip] = MIN( 0, ef2_dangle( helix[ip][0],helix[ip][1], helix[ip][0]+1,0,FALSE ) );
								}
								if( ip==0 ){
									if( helix[0][1]-helix[sum-1][0] > 1 ){
	coax[ip][ip] += MIN( 0, ef2_dangle( helix[ip][0],helix[ip][1], helix[ip][1]-1,1,FALSE ) );
									}
								}else{
	...
	coax[ip][ip] += MIN( 0, ef2_dangle( helix[ip][0],helix[ip][1], helix[ip][1]-1,1,FALSE ) );
						...
						for( ip=0; ip<sum; ip++ ){
							if( helix[ip+1][1] - helix[ip][0] == 1 ){ /* flush stacking:	*/
coax[ip][ip+1] = MIN( coax[ip][ip] + coax[ip+1][ip+1],
	ef2dp->e2_coax[rm_bcseq[helix[ip][1]]][rm_bcseq[helix[ip][0]]][rm_bcseq[helix[ip+1][1]]][rm_bcseq[helix[ip+1][0]]] );
							}else if( helix[ip+1][1] - helix[ip][0] == 2 ){ /* possible 1 ss nt	*/
								coax[ip][ip+1] = coax[ip][ip]+coax[ip+1][ip+1];
								if( ip!=0 ){
									if( helix[ip][1] - helix[ip-1][0] > 1 ){
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	ef2dp->e2_tstackcoax[rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][1]]]  [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip][1]-1]] +
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip][1]-1]][rm_bcseq[helix[ip+1][1]]][rm_bcseq[helix[ip+1][0]]] );
									}
								}else{ /* ip==0	*/
									if( helix[0][1]-helix[sum-1][0] > 1 ){
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	ef2dp->e2_tstackcoax[rm_bcseq[helix[ip][1]]]  [rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip][1]-1]] +
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip][1]-1]][rm_bcseq[helix[ip+1][1]]][rm_bcseq[helix[ip+1][0]]] );
									}
								}
								if( ip != sum-1 ){
...
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	ef2dp->e2_tstackcoax[rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]][rm_bcseq[helix[ip+1][1]]][rm_bcseq[helix[ip+1][0]]] +
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][1]]]    [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]] );
									}
								}else{ /* ip == sum-1	*/
...
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	...
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][1]]]    [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]] );
						...
						energy += MIN( coax[0][sum-1],coax[1][sum] );
					}
				}
		...
		for( k=0; k<sum; k++ ){
			memset( coax[k], 0, sum * sizeof( int ) );
		}
		...
		for( k=0; k<sum; k++ ){
			...
			coax[ip][ip] = MIN( 0, ef2_dangle( helix[ip][0],helix[ip][1], helix[ip][0]+1,0,FALSE ) );
						...
						if( rm_l_base - helix[ip][0] >= 1 ){	/* fix me! */
							/* try 3' dangle	*/
			coax[ip][ip] = MIN( 0, ef2_dangle( helix[ip][0],helix[ip][1], helix[ip][0]+1,0,FALSE ) );
						}
					}
...
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	...
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][1]]]    [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]] );
							...
							if( helix[sum-1][0]<rm_l_base ){	/* fix me */
coax[ip][ip+1] = MIN( coax[ip][ip+1],
	ef2dp->e2_tstackcoax[rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]][rm_bcseq[helix[ip+1][1]]][rm_bcseq[helix[ip+1][0]]] +
	ef2dp->e2_coaxstack [rm_bcseq[helix[ip][0]]]  [rm_bcseq[helix[ip][1]]]    [rm_bcseq[helix[ip][0]+1]][rm_bcseq[helix[ip+1][0]+1]] );
							}
		 ...
		 *			min(0,ef2_dangle(ct->basepr[count][i+1],(i+1),i,2,ct,data));
		 *		i++;
		 *	}
		 *	//j dangles on j-1
		 ...
		 *		ct->energy[count]=ct->energy[count] +
		 *			min(0,ef2_dangle(j-1,ct->basepr[count][j-1],j,1,ct,data));
		 *		j--;
		 *	}
		 ...
		 *			ef2_dangle((ct->basepr[count][k+2]),k+2,k+1,2,ct,data)) {
		 *			push (&stack,i,k+1,open,0);
		 *				push (&stack,k+2,j,open,0);
		 *		} else {
		 *			push (&stack,i,k,open,0);
...
static	int ef2_ibloop( int i, int j, int ip, int jp, int a, int b )
{
	int	energy, size, size1, size2;
	int	loginc, lopsid;		/*tlink,count,key,e[4]*/
	/*
	...
	*/
	...
	if( i<=rm_l_base && ip>rm_l_base ||
		jp<=rm_l_base && j>rm_l_base )
	{
	...
	if( size1==0||size2==0 ){
		...
		if( size==1 ){
			...
			loginc = ( int )((ef2dp->e2_prelog)*log(((double)size)/30.0));
				energy = ef2dp->e2_bulge[30] + loginc + ef2dp->e2_eparam[2];
				energy = energy + ef2_aupen(i,j) + ef2_aupen(jp,ip);
		}else{
		...
		if( size>30 ){
			...
			if( ( size1==1 || size2==1 ) && ef2dp->e2_gail ){
				energy = ef2dp->e2_tstki[rm_bcseq[i]][rm_bcseq[j]][1][1] +
					ef2dp->e2_tstki[rm_bcseq[jp]][rm_bcseq[ip]][1][1] +
					ef2dp->e2_inter[30] + loginc + ef2dp->e2_eparam[3] +
					MIN(ef2dp->e2_maxpen,(lopsid*
					ef2dp->e2_poppen[MIN(2,MIN(size1,size2))]));

			}else{
				energy = ef2dp->e2_tstki[rm_bcseq[i]][rm_bcseq[j]]
					[rm_bcseq[i+1]][rm_bcseq[j-1]] +
					ef2dp->e2_tstki[rm_bcseq[jp]][rm_bcseq[ip]]
					[rm_bcseq[jp+1]][rm_bcseq[ip-1]] +
					ef2dp->e2_inter[30] + loginc + ef2dp->e2_eparam[3] +
					MIN(ef2dp->e2_maxpen,(lopsid*
					ef2dp->e2_poppen[MIN(2,MIN(size1,size2))]));
			}
			...
			/* 2x2 internal loop	*/
			energy = ef2dp->e2_iloop22[rm_bcseq[i]][rm_bcseq[ip]]
				[rm_bcseq[j]][rm_bcseq[jp]]
				[rm_bcseq[i+1]][rm_bcseq[i+2]]
				[rm_bcseq[j-1]][rm_bcseq[j-2]];
			...
			/* energy = ef2dp->e2_stack[rm_bcseq[i]][rm_bcseq[j]]	*/
			/*	[rm_bcseq[i+1]][rm_bcseq[j-1]] +		*/
			/*	ef2dp->e2_stack[rm_bcseq[jp]][rm_bcseq[ip]]	*/
			/*	[rm_bcseq[jp+1]][rm_bcseq[ip-1]];		*/
			...
			energy = ef2dp->e2_tstki[rm_bcseq[i]][rm_bcseq[j]][rm_bcseq[i+1]][rm_bcseq[j-1]] +
...
static	int	ef2_hploop( int i, int j, int dbl )
{
	...
	if( size>30 ){
		...
		key = (rm_bcseq[j])*3125 + (rm_bcseq[i+4])*625 +
			(rm_bcseq[i+3])*125 + (rm_bcseq[i+2])*25+(rm_bcseq[i+1])*5+(rm_bcseq[i]);
		...
		energy = ef2dp->e2_tstkh[rm_bcseq[i]][rm_bcseq[j]]
			[rm_bcseq[i+1]][rm_bcseq[j-1]];
		energy = ef2dp->e2_hairpin[size] + ef2dp->e2_eparam[4] + tlink +ef2_aupen(i,j);
	}else{
		energy = ef2dp->e2_tstkh[rm_bcseq[i]][rm_bcseq[j]]
			[rm_bcseq[i+1]][rm_bcseq[j-1]] +
...
/*
		if( (i>2&&i<efn2_nbases)||(i>efn2_nbases+2) ){
*/
		if( (i>1&&i<rm_l_base)||(i>rm_l_base+2) ){
...

static	int	ef2_dangle( int i, int j, int ip, int jp, int lfce )
{
	int energy;
...

/* this function calculates whether a terminal pair i,j requires the end ef2_aupen	*/
